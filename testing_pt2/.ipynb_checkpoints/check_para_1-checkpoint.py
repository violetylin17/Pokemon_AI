...
def get_grid_state(screen, template_path=f'{goal_path}/downstairs.png'):
    ...
    gray_densities = np.zeros((grid_rows, grid_cols), dtype=float)  # To store gray density
    ...
    for row in range(grid_rows):
        for col in range(grid_cols):
            ...
            gray_densities[row, col] = gray_density  # Store gray density
    ...
    return grid_map, gray_densities

def save_tile_data(grid_map, gray_densities, filename='tile_data.txt'):
    with open(filename, 'w') as file:
        for row in range(grid_map.shape[0]):
            for col in range(grid_map.shape[1]):
                density = gray_densities[row, col]
                tile_type = grid_map[row, col]  # 0: Walkable, 1: Inaccessible, 2: Exit
                file.write(f"Tile ({row}, {col}): Gray Density: {density:.2f}, Type: {tile_type}\n")
...
def save_grid_image(grid_map):
    ...
    print("Grid image saved as 'grid_image.png'.")
...
if __name__ == "__main__":
    ...
    while True:
        screen = get_screen()
        grid, gray_densities = get_grid_state(screen)  # Get both grid and gray densities
        ...
        # Save the tile data to a text file
        save_tile_data(grid, gray_densities)
