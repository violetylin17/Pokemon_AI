...
def detect_border_by_color(screen):
    lower = np.array([68, 68, 72])   # dark gray lower bound (BGR)
    upper = np.array([88, 88, 92])   # dark gray upper bound, adjust as needed
...
def get_grid_state(screen, template_path=f'{goal_path}/downstairs.png'):
    ...
    # Detect borders on the screen
    border = detect_border_by_color(screen)
    if border:
        x, y, w, h = border
        cv2.rectangle(screen, (x, y), (x + w, y + h), (255, 0, 0), 2)  # Draw border in red
